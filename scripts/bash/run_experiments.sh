...
echo "Start time: $(date)"
python model/train/train_with_config.py --config config/config_high_accuracy.yaml
echo "High accuracy experiment completed at: $(date)"

# Experiment 2: Fast Training (Quick test)
echo "=== Experiment 2: Fast Training ==="
echo "Start time: $(date)"
python model/train/train_with_config.py --config config/config_fast_training.yaml
...
echo "Start time: $(date)"
python model/train/train_with_config.py --config config/config_experimental.yaml
...
ls -la outputs/clip_gesture_model_*.pth
ls -la outputs/clip_gesture_scaler_*.pkl
ls -la outputs/clip_*.png

echo "=== Model Sizes ==="
du -h outputs/clip_gesture_model_*.pth

echo "=== Experiment Summary ==="
echo "1. High Accuracy: outputs/clip_gesture_model_high_acc.pth"
echo "2. Fast Training: outputs/clip_gesture_model_fast.pth"
echo "3. Experimental: outputs/clip_gesture_model_experimental.pth"
