...
class CLIPGestureDataset(Dataset):
    ...
    def __init__(self, skeleton_data: torch.Tensor, gesture_labels: torch.Tensor, 
...
def prepare_clip_gesture_data(folder_path: str = "../../data",
                             max_files: int = 50,
...
def train_clip_gesture_model(X: torch.Tensor, y: torch.Tensor, gesture_names: List[str],
                           gesture_descriptions: List[str], sequence_length: int = 10,
    ...
    plt.tight_layout()
    plt.savefig('outputs/clip_confusion_matrix.png', dpi=300, bbox_inches='tight')
    ...
    plt.tight_layout()
    plt.savefig('outputs/clip_training_curves.png', dpi=300, bbox_inches='tight')
    plt.close()  # Close instead of show
    
    # Save model and scaler
    torch.save(model.state_dict(), 'outputs/clip_gesture_model.pth')
    joblib.dump(scaler, 'outputs/clip_gesture_scaler.pkl')
    ...
    with open('outputs/clip_gesture_info.json', 'w') as f:
        json.dump(gesture_info, f)
    
    print(f"\nCLIP model saved as 'outputs/clip_gesture_model.pth'")
    print(f"Scaler saved as 'outputs/clip_gesture_scaler.pkl'")
    print(f"Gesture info saved as 'outputs/clip_gesture_info.json'")
    
    return model, scaler, gesture_names, gesture_descriptions


def predict_clip_gesture(model, scaler, sequence_data: torch.Tensor, 
